...
export class AddInvitationFieldsToBusiness1755101299354 implements MigrationInterface {
    name = 'AddInvitationFieldsToBusiness1755101299354'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`businesses\` DROP COLUMN \`userId\``);
        await queryRunner.query(`ALTER TABLE \`businesses\` ADD \`invitationToken\` varchar(255) NOT NULL`);
        await queryRunner.query(`ALTER TABLE \`businesses\` ADD \`invitationTokenExpires\` timestamp NULL`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`businesses\` DROP COLUMN \`invitationTokenExpires\``);
        await queryRunner.query(`ALTER TABLE \`businesses\` DROP COLUMN \`invitationToken\``);
